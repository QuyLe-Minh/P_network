...
class TensorRTModel:
    ...
    def _setup_io_tensors(self):
        ...
        # Process all tensors
        for i in range(self.engine.num_bindings):  # Updated for TensorRT 8.5.2
    ...
    def _prepare_buffers(self, input_tensors):
        ...
        for i, tensor_name in enumerate(self.input_names):
            ...
            binding_index = self.engine.get_binding_index(tensor_name)
            shape = tuple(self.context.get_binding_shape(binding_index))
            if -1 in shape:
                self.context.set_binding_shape(binding_index, tensor.shape)
                shape = tuple(self.context.get_binding_shape(binding_index))
    ...
    def __call__(self, *input_tensors):
        ...
        for tensor_name in self.input_names:
            binding_index = self.engine.get_binding_index(tensor_name)
            self.context.set_binding_address(binding_index, self.buffer_dict[tensor_name].data_ptr())
        ...
        with torch.cuda.stream(self.stream):
            success = self.context.execute_async_v2(bindings=[buf.data_ptr() for buf in self.input_buffers + self.output_buffers],
