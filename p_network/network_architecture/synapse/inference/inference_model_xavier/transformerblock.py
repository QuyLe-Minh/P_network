...
import torch
from p_network.network_architecture.dynunet_block import UnetResBlockFused
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv51 = UnetResBlockFused(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
    ...
    def forward(self, x, posenc, global_embed = False):
        B, C, H, W, D = x.shape
        ...
        if self.pos_embed is not None:
            if global_embed:
                ...
                pos_embed = pos_embed.reshape(B, C, H * W * D).permute(0, 2, 1)
            else:
