...
from p_network.network_architecture.synapse.synapse_teacher.transformerblock import TransformerBlock
...
class SequentialWithArgs(nn.Sequential):
    def forward(self, x, posenc, *args):
        for module in self:
            x = module(x, posenc)  # Pass only `x` to each module
        return x
...
class UnetrUpBlock(nn.Module):
    ...
    def forward(self, inp, skip, posenc=None):

        out = self.transp_conv(inp)
        out = out + skip
        if isinstance(self.decoder_block[0], SequentialWithArgs):
            out = self.decoder_block[0](out, posenc)
        else:
