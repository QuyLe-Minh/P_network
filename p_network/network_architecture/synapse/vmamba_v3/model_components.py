...
from p_network.network_architecture.synapse.vmamba_v2.SS3D import VSSBlock
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32, 16, 8 , 4 ], dims=[32, 64, 128, 256],
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                ...
                stage_blocks.append(VSSBlock(dims[i], channel_first=False, spatial_one_dim_lenght=input_size[i]))
    ...
    def __init__(
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(VSSBlock(out_channels, channel_first=False, spatial_one_dim_lenght=out_size))
