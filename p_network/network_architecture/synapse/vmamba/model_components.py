...
from p_network.network_architecture.synapse.vmamba.SS3D import VSSBlock
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, dims=[32, 64, 128, 256], depths=[3, 3, 3, 3], num_heads=4, spatial_dims=3, in_channels=1, dropout=0.0, transformer_dropout_rate=0.15 ,**kwargs):
        super().__init__()
        ...
        self.GPE = nn.ModuleList()
        for i in range(4):
            stage_blocks = []
            self.GPE.append(nn.Conv3d(63, dims[i], 1))
            for j in range(depths[i]):
                ...
                stage_blocks.append(VSSBlock(dims[i], channel_first=False))
    ...
    def forward_features(self, x, posenc):
        ...
        pos_embed = self.GPE[0](posenc_prev)
        x = self.stages[0](x + pos_embed)
        ...
        for i in range(1, 4):
            ...
            pos_embed = self.GPE[i](posenc_prev)
            x = self.stages[i](x + pos_embed)
            hidden_states.append(x)
...
class UnetrUpBlock(nn.Module):
    def __init__(
        ...
        self.GPE = nn.Conv3d(63, out_channels, 1)
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                ...
                stage_blocks.append(VSSBlock(out_channels, channel_first=False))
            self.decoder_block.append(nn.Sequential(*stage_blocks))

    def forward(self, inp, skip, posenc):
        out = self.transp_conv(inp)
        out = out + skip + self.GPE(posenc)
