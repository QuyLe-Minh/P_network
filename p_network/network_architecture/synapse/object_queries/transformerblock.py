...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if cross_attn:
            self.attn_block = CrossAttn(input_size=input_size, hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads, channel_attn_drop=dropout_rate,spatial_attn_drop=dropout_rate)
        else:
            self.attn_block = EPA(input_size=input_size, hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads, channel_attn_drop=dropout_rate,spatial_attn_drop=dropout_rate)
    ...
    def forward(self, x):
        ...
        attn = x + self.gamma * self.attn_block(self.norm(x))
...
class EPA(nn.Module):
    ...
    def forward(self, x):
        ...
        q_shared = q_shared.transpose(-2, -1)   #BHCN
...
class CrossAttn(nn.Module):
    ...
    def forward(self, feats, queries):
        ...
        qk = self.qk(queries).reshape(B, N, 2, self.num_heads, C // self.num_heads)
        qk = qk.permute(2, 0, 3, 1, 4)
        vv = self.vv(feats).reshape(B, N, 2, self.num_heads, C // self.num_heads)
        vv = vv.permute(2, 0, 3, 1, 4)

        q_shared, k_shared, v_CA, v_SA = qk[0], qk[1], vv[0], vv[1]
