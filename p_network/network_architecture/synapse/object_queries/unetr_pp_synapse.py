...
from p_network.network_architecture.synapse.object_queries.model_components import UnetrPPEncoder, UnetrUpBlock, PosEnc, HashEncoding
...
class UNETR_PP(SegmentationNetwork):
    ...
    def forward(self, x_in, pos, is_posenc=False):
        if not is_posenc:
            ...
            queries = self.queries(self.posenc(pos, self.feat_size))
        else:
            posenc = pos
            queries = self.queries(F.interpolate(posenc, size=self.feat_size))
        ...
        # Four decoders
        dec3 = self.decoder5(queries, enc3, F.interpolate(posenc, scale_factor=(1/8, 1/16, 1/16)))
