...
from p_network.network_architecture.synapse.object_queries.transformerblock import TransformerBlock
...
class HashEncoding(nn.Module):
    def __init__(self, n_input_dim=3, n_levels=16):
    ...
    def forward(self, inp):
        ...
        return encoded.float().contiguous()
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(TransformerBlock(input_size=out_size, hidden_size= out_channels, proj_size=proj_size, num_heads=num_heads,
                                                     dropout_rate=0.15, pos_embed=True, cross_attn=True))
            self.decoder_block.append(nn.Sequential(*stage_blocks))

    def forward(self, queries, skip, posenc):
        queries = self.transp_conv(queries)
        queries = queries + self.GPE(posenc)
        out = self.decoder_block[0](skip, queries)
