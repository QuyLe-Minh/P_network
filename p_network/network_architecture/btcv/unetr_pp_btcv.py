...
from p_network.network_architecture.dynunet_block import UnetOutBlock, UnetResBlock
from p_network.network_architecture.btcv.model_components import UnetrPPEncoder, UnetrUpBlock, PosEnc
...
class UNETR_PP(SegmentationNetwork):
    ...
    def __init__(
        ...
        self.edge_parser = UnetOutBlock(spatial_dims=3, in_channels=feature_size, out_channels=2)
        if self.do_ds:
    ...
    def forward(self, x_in, pos, is_posenc=False, require_edge=False):
        pos=posenc=None
        if pos is not None:
            if not is_posenc:
        ...
        # dec3 = self.decoder5(dec4, enc3, F.interpolate(posenc, scale_factor=(1/16, 1/16, 1/16), mode='trilinear')) #, F.interpolate(posenc, scale_factor=(1/16, 1/16, 1/16), mode='trilinear')
        # dec2 = self.decoder4(dec3, enc2, F.interpolate(posenc, scale_factor=(1/8, 1/8, 1/8), mode='trilinear'))
        # dec1 = self.decoder3(dec2, enc1, F.interpolate(posenc, scale_factor=(1/4, 1/4, 1/4), mode='trilinear'))
