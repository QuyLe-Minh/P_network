...
from p_network.network_architecture.btcv.transformerblock import TransformerBlock
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[24 * 24 * 24, 12 * 12 * 12, 6 * 6 * 6, 3 * 3 * 3],dims=[32, 64, 128, 256],
                 proj_size =[64,64,64,16], depths=[3, 3, 3, 3],  num_heads=4, spatial_dims=3, in_channels=1, dropout=0.0, transformer_dropout_rate=0.15 ,**kwargs):
        super().__init__()
    ...
    def forward_features(self, x, posenc):
        ...
        for i in range(1, 4):
            if posenc is not None:
                if i==1:
                    posenc_prev = F.interpolate(posenc_prev, mode = "trilinear", scale_factor=(1/4,1/4,1/4))
                else:
                    posenc_prev = F.interpolate(posenc_prev, mode = "trilinear", scale_factor=(1/2,1/2,1/2))
                x = x + posenc_prev
...
class UnetrUpBlock(nn.Module):
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(TransformerBlock(input_size=out_size, hidden_size= out_channels, proj_size=proj_size, num_heads=num_heads,
                                                     dropout_rate=0.15, pos_embed=True)) #, layer_idx=j
    ...
    def forward(self, inp, skip, posenc=0):
