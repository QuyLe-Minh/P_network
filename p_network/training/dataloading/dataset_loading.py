...
from p_network.paths import preprocessing_output_dir
...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            threshold = (case_all_data[0].max() - case_all_data[0].min()) * 0.005 + case_all_data[0].min()
            bitmap = case_all_data[0].copy()
            ...
            # pos[j] = np.array([valid_bbox_x_lb/d, valid_bbox_x_ub/d, (valid_bbox_y_lb - x1)/(x2-x1), (valid_bbox_y_ub - x1)/(x2-x1), (valid_bbox_z_lb-y1)/(y2-y1), (valid_bbox_z_ub-y1)/(y2-y1)])
            pos[j] = np.array([valid_bbox_x_lb/d, valid_bbox_x_ub/d, valid_bbox_y_lb/w, valid_bbox_y_ub/w, valid_bbox_z_lb/h, valid_bbox_z_ub/h])
