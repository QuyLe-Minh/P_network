...
from p_network.network_architecture.btcv.unetr_pp_btcv import UNETR_PP
...
from p_network.training.network_training.Trainer_btcv import Trainer_btcv
...
class unetr_pp_trainer_btcv(Trainer_btcv):
    ...
    def initialize_network(self):
        ...
        self.load_pretrain('model_final_checkpoint.model')
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            ...
            if self.do_edge:
                edge_gt = self.generate_edge_tensor(target[0])
                output, edge_pred = self.network(data, pos, require_edge=True)
                ...
                edge_loss = 0
            del data
            l = self.loss(output, target) + edge_loss
            if do_backprop:
                l.backward()
                torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
                self.optimizer.step()
            ...
            splits[self.fold]['train'] = np.array(
                ['img0001', 'img0002', 'img0003', 'img0004', 'img0005', 'img0006', 'img0007', 'img0008', 'img0009',
                 'img0010', 'img0021', 'img0022' \
                    , 'img0023', 'img0024', 'img0025', 'img0026', 'img0027', 'img0028', 'img0029', 'img0030', 'img0031',\
                        'img0032', 'img0033', 'img0034'])
            splits[self.fold]['val'] = np.array(
                ['img0035', 'img0036', 'img0037', 'img0038', 'img0039', 'img0040'])
            # splits[self.fold]['val'] = np.array(
            #     [f'img00{n}' for n in range(61, 81)])
