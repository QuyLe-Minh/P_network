...
from p_network.network_architecture.synapse.synapse_teacher.unetr_pp_synapse import UNETR_PP
...
class unetr_pp_trainer_synapse_teacher(Trainer_synapse):
    """
    ...
    def initialize_network(self):
        ...
        self.load_pretrain("unetrpp_synapse_teacher.model")
        ...
        edge_map = torch.ones(()).new_empty((1, *input_res), dtype=next(self.network.parameters()).dtype,
                                 device=next(self.network.parameters()).device)
        pos = torch.ones(()).new_empty((1, 6), dtype=next(self.network.parameters()).dtype,
                                         device=next(self.network.parameters()).device)
        # flops = FlopCountAnalysis(self.network, (input, pos, edge_map))
    ...
    def load_pretrain(self, path):
        ...
        not_included = ["unetr_pp_encoder.downsample_layers.0.1.weight", "unetr_pp_encoder.downsample_layers.0.1.bias", "unetr_pp_encoder.downsample_layers.0.0.conv.weight"]
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            ...
            with torch.no_grad():
                ...
                data = data + 10*edge_gt
                data = data.clip(mn, mx)
            output, _ = self.network(data, pos, require_feature=False)
            del data
            l = self.loss(output, target)
            if do_backprop:
                l.backward()
