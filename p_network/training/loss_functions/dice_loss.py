...
from p_network.training.loss_functions.TopK_loss import TopKLoss
from p_network.training.loss_functions.crossentropy import RobustCrossEntropyLoss, RobustPolyCrossEntropyLoss
from p_network.utilities.nd_softmax import softmax_helper
from p_network.utilities.tensor_utilities import sum_tensor
...
class DC_and_Focal_loss(nn.Module):
    ...
    def forward(self, net_output, target, weight_loss_map=None):
        """
        ...
        dc_loss = self.dc(net_output, target, loss_mask=weight_loss_map.unsqueeze(1)) if self.weight_dice != 0 else 0
        if self.log_dice:
            dc_loss = -torch.log(-dc_loss)

        focal_loss = self.focal(net_output, target.long()) if self.weight_ce != 0 else 0
        if self.ignore_label is not None:
            focal_loss *= mask[:, 0]
...
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
        ...
        self.ce = RobustPolyCrossEntropyLoss(reduction = 'none', **ce_kwargs)
    ...
    def forward(self, net_output, target, weight_loss_map=None):
        ...
        ce_loss = self.ce(net_output, target[:, 0].long(), weight_loss_map) if self.weight_ce != 0 else 0
        if self.ignore_label is not None:
            ce_loss *= mask[:, 0]
