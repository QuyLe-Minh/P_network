...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        h = w = d = int(input_size ** (1 / 3))+1
        self.norm = nn.LayerNorm(hidden_size)
        self.gamma = nn.Parameter(1e-6 * torch.ones(hidden_size), requires_grad=True)
        self.c_epa_block = E_EPA(input_size=input_size, hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads, channel_attn_drop=dropout_rate,spatial_attn_drop=dropout_rate)
        self.d_epa_block = CausalAttentionBlock(hidden_size=h*w*hidden_size, num_heads=num_heads, attn_drop=dropout_rate)
        ...
        self.pos_embed = nn.Conv3d(63, hidden_size, 1)

    def forward(self, x, posenc):
        B, C, H, W, D = x.shape
        
        pos_embed = self.pos_embed(posenc)
        x = x + pos_embed
        x = x.reshape(B, C, -1).permute(0, 2, 1)
        
        x = self.norm(x)
        c_attention = self.c_epa_block(x)
        
        inp_d = x.reshape(B, D, H, W, C).reshape(B, D, -1) # BDHWC -> BD(HWC)
        d_attention = self.d_epa_block(inp_d)
        d_attention = d_attention.reshape(B, D, H, W, C).reshape(B, -1, C)
        
        attn = x + self.gamma * (c_attention + d_attention)
...
class CausalAttentionBlock(nn.Module):
    def __init__(self,  hidden_size, num_heads=4, qkv_bias=False, attn_drop=0.1):
        super().__init__()
        self.num_heads = num_heads
        self.hidden_size = hidden_size
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
        self.down_project_channel = nn.Linear(hidden_size, 256)
        self.qkv = nn.Linear(256, 256 * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.out_proj = nn.Linear(256, hidden_size)
        
    def forward(self, x):
        B, N, _ = x.shape
        C = 256
        x_projected = self.down_project_channel(x)
        qkv = self.qkv(x_projected).reshape(B, N, 3, self.num_heads, C // self.num_heads)
