...
class PosEnc(nn.Module):
    ...
    def forward(self, pos, inp_shape):
        ...
        z = torch.linspace(-1, 1, w, device=device)
        grid = torch.stack(torch.meshgrid(x, y, z, indexing='ij'), dim=-1)  # (d, h, w, 3)
        grid = grid.unsqueeze(0).repeat(B, 1, 1, 1, 1)  # (B, d, h, w, 3)

        # Apply per-sample scaling and shifting
        lb = pos[:, [0, 2, 4]].view(B, 1, 1, 1, 3)
        ub = pos[:, [1, 3, 5]].view(B, 1, 1, 1, 3)
        grid = (grid + 1) / 2 * (ub - lb) + lb  # scale grid to [lb, ub]

        mesh_grids = self.embedding(grid)
        mesh_grids = mesh_grids.permute(0, 4, 1, 2, 3)
        return mesh_grids.contiguous()
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        coords_h = torch.arange(self.window_size[1])
        coords_w = torch.arange(self.window_size[2])
        coords = torch.stack(torch.meshgrid([coords_s, coords_h, coords_w]))    #3,4,4,4
        coords_flatten = torch.flatten(coords, 1)   # 3, 4**3
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  
        relative_coords = relative_coords.permute(1, 2, 0).contiguous() 
        relative_coords[:, :, 0] += self.window_size[0] - 1  # shift to start from 0
        relative_coords[:, :, 1] += self.window_size[1] - 1
        ...
        relative_position_index = relative_coords.sum(-1) #4**3, 4**3, 1
        self.register_buffer("relative_position_index", relative_position_index)
    ...
    def forward(self, x, mask=None, pos_embed=None):
        ...
        qkv=qkv.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous() #BHNC
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1).contiguous())   #BHNN
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1] * self.window_size[2],
            self.window_size[0] * self.window_size[1] * self.window_size[2], -1)  
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous() 
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        # cyclic shift
        # @TODO: Rolling positional encoding here
        if self.shift_size > 0:
        ...
        # reverse cyclic shift
        # @TODO: Reverse rolling positional encoding here
        if self.shift_size > 0:
            x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size, self.shift_size), dims=(1, 2, 3))
        else:
            x = shifted_x
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.position = nn.Conv3d(63, dim, 1)
    ...
    def forward(self, x, posenc, S, H, W):
        ...
        posembed = self.position(posenc)
        posembed = posembed.view(posembed.shape[0], posembed.shape[1], -1).permute(0,2,1).contiguous()
        for blk in self.blocks:
            x = blk(x+posembed, attn_mask)
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, skip, posenc, S, H, W):  
        ...
        mask_windows = mask_windows.view(-1, self.window_size * self.window_size * self.window_size)  # 3d��3��winds�˻�����Ŀ�Ǻܴ�ģ�����winds����̫��
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
        ...
        x = self.blocks[0](x+posembed, attn_mask,skip=skip,x_up=x_up)
        for i in range(self.depth-1):
            x = self.blocks[i+1](x+posembed,attn_mask)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, posenc):
        ...
        posenc_cp = F.interpolate(posenc, size=(Ws, Wh, Ww))
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, S, H, W, x, Ws, Wh, Ww = layer(x, posenc_cp, Ws, Wh, Ww)
            posenc_cp = F.interpolate(posenc, size=(Ws, Wh, Ww))
...
class Decoder(nn.Module):
    ...
    def forward(self, x, skips, posenc):
        outs=[]
        ...
        posenc_cp = F.interpolate(posenc, size=(S*2, H*2, W*2)) #after upsampling it will be added to attention
        for i in range(self.num_layers)[::-1]:
            layer = self.layers[i]
            x, S, H, W = layer(x, skips[i], posenc_cp, S, H, W)
            posenc_cp = F.interpolate(posenc_cp, size=(S*2, H*2, W*2))
