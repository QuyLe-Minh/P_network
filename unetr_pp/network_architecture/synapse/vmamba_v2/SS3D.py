...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.ssm_branch:
            ...
            self.norm2 = norm_layer(spatial_one_dim_lenght)
            self.norm3 = norm_layer(spatial_one_dim_lenght)
            self.norm4 = norm_layer(spatial_one_dim_lenght)
            self.op1 = _SS3D(
                d_model=hidden_dim, 
                d_state=ssm_d_state, 
                ssm_ratio=ssm_ratio,
            ...
            self.op2 = _SS3D(
                d_model=spatial_one_dim_lenght, 
                d_state=ssm_d_state*32//spatial_one_dim_lenght, 
                ssm_ratio=ssm_ratio,
                dt_rank=ssm_dt_rank,
                act_layer=ssm_act_layer,
                # ==========================
    ...
    def forward(self, input: torch.Tensor):
        ...
        x = x1
        x2 = input.permute(0, 1, 3, 4, 2).contiguous()
        x3 = input.permute(0, 1, 2, 4, 3).contiguous()
        x4 = input
        
        if self.ssm_branch:
            x1 = self.op1(self.norm1(x1)) * self.gamma[0]
            x2 = self.op2(self.norm2(x2)).permute(0, 4, 2, 3, 1).contiguous() * self.gamma[1]
            x3 = self.op3(self.norm3(x3)).permute(0, 2, 4, 3, 1).contiguous() * self.gamma[2]
            x4 = self.op4(self.norm4(x4)).permute(0, 2, 3, 4, 1).contiguous() * self.gamma[3]
