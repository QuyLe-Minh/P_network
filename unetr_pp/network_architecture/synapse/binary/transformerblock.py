...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.epa_block = EPA(hidden_size=hidden_size, num_heads=num_heads, spatial_attn_drop=dropout_rate)
    ...
    def forward(self, x, bank):
        ...
        attn = x + self.gamma * self.epa_block(self.norm(x), bank)
...
class EPA(nn.Module):
    ...
    def forward(self, x, bank):
        ...
        attn = (q.unsqueeze(0) @ k.unsqueeze(1).transpose(-2, -1)) * self.temperature #N_classes, B, H, N, N_bank
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        x = (attn @ v.unsqueeze(1)).sum(0).permute(0, 2, 1, 3).reshape(B, N, C)
