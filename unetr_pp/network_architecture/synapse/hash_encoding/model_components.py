...
from unetr_pp.network_architecture.synapse.hash_encoding.transformerblock import TransformerBlock
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4], dims=[32, 64, 128, 256],
        ...
        for i in range(4):
            stage_blocks = []
            self.hash.append(nn.Sequential(HashEncoding(), nn.Conv3d(32, dims[i], 1)))
