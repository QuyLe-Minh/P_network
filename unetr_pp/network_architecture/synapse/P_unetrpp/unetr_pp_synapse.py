...
from unetr_pp.network_architecture.dynunet_block import UnetOutBlock, UnetResBlock, GathernExcite
from unetr_pp.network_architecture.synapse.P_unetrpp.model_components import UnetrPPEncoder, UnetrUpBlock, PosEnc
...
class UNETR_PP(SegmentationNetwork):
    ...
    def forward(self, x_in, pos, require_density=None, is_posenc=False):
        if not is_posenc:
        ...
        if not require_density:
            return logits
        else:
            density = self.density(enc4)
...
if __name__ == '__main__':
    import torch
    net = UNETR_PP(in_channels=1, out_channels=14, img_size=(64, 128, 128), feature_size=16, num_heads=4,
                  depths=[3, 3, 3, 3], dims=[32, 64, 128, 256], do_ds=False).cuda()
    x = torch.randn(1, 1, 64, 128, 128).cuda()
    pos = torch.ones(()).new_empty((1, 6), dtype=next(net.parameters()).dtype,
                                        device=next(net.parameters()).device)
    y = net(x, pos)
    net.eval()
    
    with torch.no_grad():
        for i in range(3*4*4*8):
            net(x)