...
from unetr_pp.network_architecture.synapse.hash_v2_sota.transformerblock import TransformerBlock
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4],dims=[32, 64, 128, 256],
        ...
        for i in range(4):
            stage_blocks = []
            self.hash.append(nn.Sequential(HashEncoding(), nn.Conv3d(64, dims[i], 1)))
    ...
    def forward_features(self, x, posenc):
        ...
        pos_embed = self.hash[0](posenc_prev)
        ...
        for i in range(1, 4):
            ...
            pos_embed = self.hash[i](posenc_prev)
...
class UnetrUpBlock(nn.Module):
    def __init__(
        ...
        self.last_decoder = conv_decoder
    ...
    def forward(self, inp, skip, posenc):
        ...
        out = out + skip + self.hash(posenc)
        # if not self.conv_decoder:
